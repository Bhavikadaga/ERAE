      ...
      <nav className="relative bg-white border-b border-stone-200 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between relative">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:flex items-center gap-8">
              <Link to="/products/new-arrivals" className="text-xs tracking-widest uppercase text-stone-500 hover:text-stone-800 transition-colors">
                New Arrivals
              </Link>
              ...
              <div className="relative group">
                ...
                <div className="absolute top-full left-0 mt-2 w-48 bg-white border border-stone-200 shadow-md opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                  ...
                  {categories.length === 0 && (
                    <p className="px-4 py-3 text-xs text-stone-400">No categories</p>
                  )}
                </div>
              </div>

              <Link to="/products/sale" className="text-xs tracking-widest uppercase text-red-400 hover:text-red-600 transition-colors">
                Sale
              </Link>
            </div>
          </div>
            ...
            <Link to="/cart" className="relative">
              ...
              {cartCount > 0 && (
                <span
                  ...
                  style={{ backgroundColor: settings.accentColor, fontSize: '10px' }}
                >
                  {cartCount}
                </span>
              )}
        ...
        {menuOpen && (
          <div className="md:hidden mt-4 flex flex-col gap-4 px-2 pb-4 border-t border-stone-100 pt-4">
            <Link to="/products/new-arrivals" onClick={() => setMenuOpen(false)} className="text-xs tracking-widest uppercase text-stone-500">
              New Arrivals
            </Link>
            ...
            <Link to="/products/sale" onClick={() => setMenuOpen(false)} className="text-xs tracking-widest uppercase text-red-400">
              Sale
            </Link>
